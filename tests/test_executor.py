...
def test_identifier_numeric_column_excluded_from_describe():
    df = pd.DataFrame({"participant_id": [10001, 10002, 10003], "score": [1.0, 2.0, 3.0]})
...
def test_small_groups_are_suppressed():
    df = pd.DataFrame({"group": ["A"] * 4 + ["B"] * 6, "score": list(range(10))})
    profiles = profile_dataframe(df)
    result = group_summary(df, profiles, "group", "score", min_group_size=5)
    assert [g["group"] for g in result["groups"]] == ["B"]


def test_identifier_cannot_be_grouped():
    df = pd.DataFrame({"subject_id": [1, 2, 3, 4, 5], "score": [1, 2, 3, 4, 5]})
    profiles = profile_dataframe(df)
    with pytest.raises(ValueError):
        group_summary(df, profiles, "subject_id", "score")
